...
use crate::{
    error::ErrorCode,
    states::{PoolConfig, PoolState},
    util,
};

#[derive(account)]
pub struct ListToRay<'info> {
    ...
    #[account(mut)]
    pub g_pool: AccountLoader<'info, PoolState>, // stands for gate_pool

    pub g_pool_config: AccountLoader<'info, PoolConfig>, // stands for gate_pool_config
    ...
    pub pool_state: UncheckedAccount<'info>, // init by cp-swap
...
pub fn list_to_ray(
    ctx: Context<ListToRay>,
    init_amount_0: u64,
    init_amount_1: u64,
    open_time: u64,
    token0_is_native_mint: bool,
) -> Result<()> {
    let g_pool_info = ctx.accounts.g_pool.load()?;
    let g_pool_config = ctx.accounts.g_pool_config.load()?;
    require_keys_eq!(
        g_pool_info.pool_config.key(),
        ctx.accounts.g_pool_config.key()
    );

    require_keys_eq!(
        g_pool_config.list_admin.key(),
        ctx.accounts.creator.to_account_info().key()
    );

    require!(
        g_pool_info.get_is_ready_to_list(),
        ErrorCode::PoolNotReadyToList
    );

    // convert sol to wsol
    if token0_is_native_mint {
        util::user_wrap_sol(
    ...
    // msg!("Expected true, and ->: {:?}",ctx.accounts.token_0_mint.key() < ctx.accounts.token_1_mint.key());
